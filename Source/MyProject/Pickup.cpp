...
void APickup::BeginPlay()
{
	...
	if (Components.Num() > 0) {
		for (auto& Comp : Components) {
			HoldComp = Cast<USceneComponent>(Comp);
		}
	}
}
...
void APickup::Tick(float DeltaTime)
{
	...
	if (bHolding && HoldComp) {
		SetActorLocationAndRotation(HoldComp->GetComponentLocation(), HoldComp->GetComponentRotation());
	}
}
...
void APickup::Pickup()
{
	bHolding = !bHolding; bGravity = !bGravity;
